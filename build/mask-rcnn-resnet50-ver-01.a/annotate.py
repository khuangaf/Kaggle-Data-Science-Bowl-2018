...
def run_make_test_annotation_2():

    split = 'test2_ids_all_3019'
    ids = read_list_from_file(DATA_DIR + '/split/' + split, comment='#')
    data_dir = DATA_DIR + '/image/stage2_test_final'
    os.makedirs(data_dir + '/images', exist_ok=True)
    num_ids = len(ids)
...
def run_make_external_annotation():

    split = 'external_ids_colored_30'

    ids = read_list_from_file(DATA_DIR + '/split/' + split, comment='#')

    data_dir = DATA_DIR + '/image/external'
    os.makedirs(data_dir + '/multi_masks', exist_ok=True)
    os.makedirs(data_dir + '/overlays', exist_ok=True)
    os.makedirs(data_dir + '/images', exist_ok=True)
    ...
    for i in range(num_ids):
        ...
        name   = id.split('/')[-1]
        folder = id.split('/')[0]
        image_files = glob.glob(DATA_DIR + '/__download__/%s/%s/images/*.tif'%(folder,name))
        assert(len(image_files)==1)
        image_file=image_files[0]
#         print(id)
